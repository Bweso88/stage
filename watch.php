...
.sitem-play-icon{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.3);}
...
<div class="breadcrumb-bar">
  <div class="breadcrumb-custom">
    ...
    <a href="index.php#formations">Formations</a><span class="breadcrumb-sep">›</span>
    <a href="videos.php?theme=<?php echo urlencode($theme); ?>"><?php echo htmlspecialchars($theme); ?></a><span class="breadcrumb-sep">›</span>
...
<div class="watch-layout">
  <div>
    ...
    <div class="video-info">
      ...
      <div class="video-meta-row">
        <div class="meta-pill"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg><?php echo $views; ?> vue(s)</div>
        <div class="meta-pill"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg><?php echo htmlspecialchars($theme); ?></div>
        <div class="meta-pill"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg><?php echo count($allVideos); ?> vidéo(s) dans ce thème</div>
        <a href="videos.php?theme=<?php echo urlencode($theme); ?>" class="back-link"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>Retour au thème</a>
      </div>
    </div>
  </div>
  ...
  <aside class="watch-sidebar">
    <div class="sidebar-head">
      📁 <span><?php echo htmlspecialchars($theme); ?></span>&nbsp;— <?php echo count($allVideos); ?> vidéo(s)
    </div>
    <?php foreach ($allVideos as $idx => $v):
      $f=basename($v);$fn=pathinfo($f,PATHINFO_FILENAME);
      $vvf=$viewsDir.$fn.".txt";$vc=file_exists($vvf)?(int)file_get_contents($vvf):0;
      $lbl=ucfirst(str_replace(['_','-'],' ',$fn));$isCurrent=($f===$file);
    ?>
    <a href="watch.php?theme=<?php echo urlencode($theme); ?>&video=<?php echo urlencode($f); ?>" class="sidebar-item <?php echo $isCurrent?'active-vid':''; ?>">
      <div class="sitem-thumb">
        <video preload="metadata" muted><source src="<?php echo htmlspecialchars($v); ?>#t=0.1" type="video/mp4"></video>
        <?php if (!$isCurrent): ?><div class="sitem-play-icon"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></div><?php endif; ?>
      </div>
      <div class="sitem-info">
        ...
        <div class="sitem-views"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg><?php echo $vc; ?> vue(s)</div>
      </div>
    </a>
    <?php endforeach; ?>
  </aside>
...
(function(){
  ...
  function send(sec){navigator.sendBeacon('track_video.php',new URLSearchParams({log_id:logId,seconds:Math.round(sec)}));}
